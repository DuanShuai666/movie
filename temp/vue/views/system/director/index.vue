<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="directorList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="${comment}" align="center" prop="movieId" />
      <el-table-column label="${comment}" align="center" prop="personId" />
...
<script>
import { listDirector, getDirector, delDirector, addDirector, updateDirector } from "@/api/system/director"
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除导演编号为"' + movieIds + '"的数据项？').then(function() {
        return delDirector(movieIds)
      }).then(() => {
        this.getList()
